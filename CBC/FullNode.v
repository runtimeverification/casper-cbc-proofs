...
From CasperCBC
Require Import Lib.Preamble Lib.ListExtras Lib.ListSetExtras Lib.RealsExtras VLSM.CBC.Basic VLSM.CBC.SumWeights CBC.Protocol CBC.Common CBC.Definitions.
...
Definition exists_pivotal_validator_ps
  ...
  :=
  @exists_pivotal_validator V (@strictly_comparable_eq_dec _ (triple_strictly_comparable_proj2 about_M)) Hm Hrt.
    ...
    + destruct (distinct_choice_total v) as [v' Hv'].
      ...
      { apply Rlt_le_trans with (fault_weight_state (add (c, v, Empty) to (add (c0, v, sigma0) to Empty))).
        - unfold fault_weight_state. unfold equivocating_senders. simpl.
          ...
          apply (Rplus_gt_compat_r (proj1_sig (SumWeights.weight v))) in Hgt. rewrite Rplus_assoc in Hgt.
    ...
    + remember (add_in_sorted_fn (c, v, Empty) Empty) as sigma.
      ...
      { apply (not_heavy_subset (add (c0, v, sigma0) to sigma2)) in Hps'.
        - apply Rlt_irrefl with (proj1_sig threshold).
          ...
          unfold Rminus in Hgt. apply (Rplus_gt_compat_r (proj1_sig (SumWeights.weight v))) in Hgt.
          rewrite Rplus_assoc in Hgt. rewrite Rplus_opp_l, Rplus_0_r in Hgt.
          apply Rgt_lt. apply Rge_gt_trans with (sum_weights (v' :: vs) + (proj1_sig (SumWeights.weight v)))%R; try assumption.
          rewrite Rplus_comm.
          assert (Hsum : (proj1_sig (SumWeights.weight v) + sum_weights (v' :: vs))%R = sum_weights (v :: v' :: vs))
...
Definition add_weight_under
  ...
  (s : @state C V) (v : V)
  :=
  (fault_weight_state s + proj1_sig (SumWeights.weight v) <= proj1_sig threshold)%R.
...
Proof.
  ...
  - (* Induction step *)
    ...
    apply (Rplus_le_reg_pos_r (fault_weight_state s + sum_weights tl) (proj1_sig (SumWeights.weight hd)) (proj1_sig threshold)).
    destruct (SumWeights.weight hd). firstorder.
    rewrite Rplus_assoc.
    rewrite (Rplus_comm (sum_weights tl) (proj1_sig (SumWeights.weight hd))).
    rewrite <- Rplus_assoc.
    ...
    + simpl.
      ...
      assumption.
    + simpl.
      rewrite (Rplus_comm (proj1_sig (SumWeights.weight hd)) (sum_weights tl)).
      rewrite <- Rplus_assoc.
      rewrite <- H_weight.
...
Definition potentially_pivotal_state
  ...
  exists (vs : list V),
    ...
    (sum_weights ((equivocating_senders s) ++ vs) >
     proj1_sig threshold - proj1_sig (SumWeights.weight v))%R.
...
Proof.
  ...
  - (* Proving that s2 and s3 don't share a common future *)
    ...
    assert (H_s_overweight : (proj1_sig (SumWeights.weight v) + fault_weight_state (proj1_sig s2) + sum_weights vs <= fault_weight_state s)%R).
    { replace ((proj1_sig (SumWeights.weight v) + fault_weight_state (proj1_sig s2) + sum_weights vs))%R with (sum_weights ([v] ++ (equivocating_senders (proj1_sig s2)) ++ vs)).
    ...
    apply (Rplus_gt_compat_l (proj1_sig (SumWeights.weight v))) in H_over.
    replace (proj1_sig (SumWeights.weight v) + (proj1_sig threshold - proj1_sig (SumWeights.weight v)))%R with (proj1_sig threshold)%R in H_over by ring.
