...
Definition equivocators_transition_item_project
  ...
  match item with {| l := el; input := iom; output := oom; destination := s |} =>
    ...
    let (i, li) := el in
    match i as n return (vlabel (equivocator_IM n) -> option (option (vtransition_item X) * equivocators_choice)) with
    | inl eqv =>
      ...
      fun li0  =>
        match
          equivocator_vlsm_transition_item_project
            (IM (inl eqv))
            {| l := li0; input := iom; output := oom; destination := s (inl eqv) |}
            deqv
            with
        | Some (Some item', deqv') =>
          Some
            (Some (@Build_transition_item message (@type message X)
              (existT (fun n : index => vlabel (IM n)) (inl eqv) (l item'))
              iom sx oom)
    ...
    | inr neqv =>
        fun li0  => Some
          (Some (@Build_transition_item message (@type message X)
            (existT (fun n : index => vlabel (IM n)) (inr neqv) li0)
            iom sx oom)
          , eqv_choice)
    end li
  end.
...
Proof.
  ...
  destruct l.
  intro contra.
  destruct x; [|discriminate contra].
  spec Hproper e. simpl in Hproper.
  elim
    (equivocator_transition_item_project_proper (IM (inl e))
    (@Build_transition_item message
    (@equivocator_type message
       (IM (@inl equiv_index nequiv_index e))) v input
    (destination (@inl equiv_index nequiv_index e)) output)
  ...
  destruct
    ...
    (@Build_transition_item message
                  (@equivocator_type message
                     (IM (@inl equiv_index nequiv_index e))) v input
                  (destination (@inl equiv_index nequiv_index e)) output) (eqv_choice e))
    ; [|reflexivity].
...
Proof.
  ...
  - unfold equivocators_trace_project. simpl.
    ...
    destruct l as (i, li).
    destruct (vtransition (Common.equivocator_IM IM i) li (is i, iom)) as (si', om')
      eqn:Ht'.
    ...
    destruct i as [eqv | neqv]; simpl in *.
    + destruct (initial_choice' eqv) as [seqv | ieqv feqv] eqn:Heqv.
      * eexists _. eexists _. exists eq_refl.
        ...
        -- replace
            ...
            (equivocators_choice_update initial_choice' eqv
               (NewMachine (IM (inl eqv)) seqv)) is)
            with
              (equivocators_state_project initial_choice'
              (state_update (Common.equivocator_IM IM) is (inl eqv) si'))
            ;[assumption|].
          assert
            (Hinitial_choice' : equivocators_choice_update initial_choice' eqv
            (NewMachine (IM (inl eqv)) seqv) = initial_choice').
        ...
        -- intro eqv'.
          ...
          ++ unfold equivocators_choice_update. rewrite equivocators_choice_update_neq; try assumption.
            spec Hproper' eqv'.
            rewrite state_update_neq in Hproper'; try assumption. congruence.
        -- replace
            (equivocators_state_project
            (equivocators_choice_update initial_choice' eqv
               (NewMachine (IM (inl eqv)) seqv)) is)
          ...
          apply functional_extensionality_dep_good.
          intro i'. destruct (decide (i' = (inl eqv))).
          ...
          ++ rewrite state_update_neq; try assumption.
            unfold equivocators_state_project.
            unfold Common.equivocators_state_project.
            destruct i' as [eqv' | neqv']; try reflexivity.
            unfold equivocators_choice_update.
      ...
      * rewrite state_update_eq.
        ...
        unfold vtransition in Ht'. unfold transition in Ht'.
        ...
        -- destruct Hv as [Hsdi _Hiom]. subst iom.
          ...
          destruct (is (inl eqv)) as (is_eqv_n, is_eqv_s) eqn:His_eqv.
          ...
          ++ inversion e0. subst ieqv. clear e e0.
            ...
            assert
              ...
              (last (map destination tr)
                 (state_update (Common.equivocator_IM IM) is (inl eqv)
                    (equivocator_state_extend (IM (inl eqv)) (existT (fun n : nat => t (S n) -> vstate (IM (inl eqv)))
                    is_eqv_n is_eqv_s) sdi))) =
              last (map destination trX')
                (equivocators_state_project
                   (equivocators_choice_update initial_choice' eqv
                      (NewMachine (IM (inl eqv)) sdi)) is)
              ).
            { subst bsi'. simpl in *. rewrite Hfinal.
              ...
              - rewrite state_update_neq; try assumption.
                unfold equivocators_state_project.
                unfold Common.equivocators_state_project.
                destruct i' as [eqv' | neqv']; [|reflexivity].
                unfold equivocators_choice_update.
            ...
            unfold equivocators_choice_update.
            destruct (decide (i = (inl eqv))).
            ** subst. rewrite state_update_eq.
              rewrite equivocators_choice_update_eq.
              reflexivity.
            ** repeat rewrite state_update_neq by assumption.
              unfold Common.equivocators_state_project.
              destruct i as [eqvi | neqvi]; [|reflexivity].
              rewrite equivocators_choice_update_neq; congruence.
          ++ eexists _. eexists _. exists eq_refl.
            assert
              (Hfinal' :
                ...
                last (map destination trX')
                  (equivocators_state_project
                     (equivocators_choice_update initial_choice' eqv
                        (Existing (IM (inl eqv)) ieqv false)) is)
              ).
            { subst bsi'. simpl in *. rewrite Hfinal.
              rewrite (equivocators_state_project_state_update_eqv IM i0).
              rewrite Heqv.
      ...
      -- destruct Hv as [Hidi Hv].
        destruct (le_lt_dec (S (projT1 (is (inl eqv)))) idi); [lia|].
        replace (of_nat_lt l0) with (of_nat_lt Hidi) in * by apply of_nat_ext.
        clear l0.
        unfold fst in Ht'.
        destruct (is (inl eqv)) as (neqv, bseqv) eqn:Hiseqv.
        unfold projT2 in Ht'.
        destruct (vtransition (IM (inl eqv)) li (bseqv (of_nat_lt Hidi), iom))
          as (si', siom') eqn:Ht''.
        destruct fdi.
        ++ inversion Ht'. subst.
          simpl_existT.
          ...
          ** eexists _. eexists _. exists eq_refl.
            ...
            assert
              (Hfinal' :
                ...
                last
                  ...
                  (equivocators_state_project
                     (equivocators_choice_update initial_choice' eqv
                        (Existing (IM (inl eqv)) idi true)) is)
              ).
            ...
            --- unfold equivocators_state_project.
              ...
              +++ repeat rewrite state_update_neq; try assumption.
                unfold  Common.equivocators_state_project.
                destruct i as [eqvi | neqvi]; try reflexivity.
          ...
          ** eexists _. eexists _. exists eq_refl.
            ...
            --- subst. rewrite state_update_eq.
              rewrite to_nat_of_nat.
              destruct (nat_eq_dec ieqv (S neqv)); try congruence.
              unfold Common.equivocators_state_project.
              unfold equivocators_choice_update.
              rewrite equivocators_choice_update_eq.
              rewrite Hiseqv.
        ...
        ++ inversion Ht'. subst. simpl_existT.
          ...
          --- subst. simpl in Hidi.
              ...
              (Hfinal' :
                equivocators_state_project final_choice
                  (last (map destination tr)
                     (state_update (Common.equivocator_IM IM) is
                        (inl eqv)
                        (existT (fun n : nat => t (S n) -> vstate (IM (inl eqv))) nsi'
                           (fun j : t (S nsi') =>
            ...
            +++ unfold equivocators_state_project.
              ...
              destruct (le_lt_dec (S nsi') ieqv). { lia. }
              replace (of_nat_lt l) with (of_nat_lt Hidi) in * by apply of_nat_ext.
              clear l. clear Hproject. subst. rewrite eq_dec_if_true; try reflexivity.
              simpl in *. rewrite Ht''.
              f_equal.
              apply functional_extensionality_dep_good.
              intro i.
  ...
  + eexists _. eexists _. exists eq_refl.
    ...
    assert
      (Hfinal' :
        equivocators_state_project final_choice
          (last (map destination tr)
             (state_update (Common.equivocator_IM IM) is (inr neqv) si')) =
        last
          (equivocators_state_project initial_choice'
             (state_update (Common.equivocator_IM IM) is (inr neqv) si')
           :: map destination trX')
          (equivocators_state_project initial_choice' is)
      ).
    { rewrite unroll_last. simpl in *. rewrite Hfinal. reflexivity. }
    exists Hfinal'.
    apply proper_equivocators_choice_state_update_neqv in Hproper'.
    exists Hproper'.
    constructor; try assumption.
    repeat split; try assumption.
    * apply His. assumption.
