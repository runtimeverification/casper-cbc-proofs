...
Proof.
  ...
  destruct (eqv_choice x) as [|eqv_i eqv_f] eqn:Heq_eqv
...
Proof.
  ...
  destruct Hinit as [Hproject Hsize].
 specialize
  (equivocators_trace_project_skip_reset_trace
    _ _ Heqv_choice tr is_final Hsize
  )
  as Htrace.
  ...
  match goal with
  |- fold_right _ ?f _ = _ => replace f with (Some (@nil (vtransition_item X), eqv_choice))
  end.
  assumption.
Qed.
...
Proof.
  induction HrunX.
  - specialize (lift_initial_to_equivocators_state IM Hbs i0 index_listing finite_index is His) as Hs.
    remember (lift_to_equivocators_state IM i0 is) as s.
  ...
  - destruct IHHrunX1 as [eqv_state_run [Heqv_state_run [eqv_state_choice [Heqv_state_choice [Hstate_project [Hstate_final_project [_ Hstate_start_project]]]]]]].
    ...
    specialize
      ...
      as Hmsg_trace_reset.
    simpl in Hmsg_trace_reset.
    match type of Hmsg_trace_reset with
    ...
    apply proj1 in Happ.
    specialize (Happ (conj (proj1 Hstate_trace) Hmsg_trace_reset)).
    simpl.
    specialize
    ...
    assert (Hnext_state_choice : not_equivocating_equivocators_choice eqv_state_choice es').
    ...
    { subst last_prj. subst last_item.
      ...
      simpl.
      repeat f_equal.
      apply equivocators_choice_update_id.
      assumption.
    }
    rewrite H.
    ...
    {
      subst emsg_tr.
      apply equivocators_trace_project_skip_reset_trace_full. assumption.
    }
    specialize
      (equivocators_trace_project_folder_additive IM Hbs i0 _ finite_index
