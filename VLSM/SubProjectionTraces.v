...
From CasperCBC
Require Import Lib.ListExtras Lib.Preamble VLSM.Common VLSM.Composition.

Section SubProjectionTraces.
...
Context
  ...
  (sub_index : Type := sub_index sub_index_list)
  (sub_index_eq_dec := sub_index_eq_dec sub_index_list)
  (sub_index_prop := sub_index_prop sub_index_list)
  (sub_index_prop_dec := sub_index_prop_dec sub_index_list)
  (sub_IM : sub_index -> VLSM message := sub_IM IM sub_index_list)
  .

Existing Instance sub_index_prop_dec.
...
Proof.
  ...
  simpl in *. f_equal.
  match goal with
  |- existT _ ?x1 _ = existT _ ?x2 _ => assert (Heq : x1 = x2)
  end.
  { apply dec_sig_eq_iff. reflexivity. }
  simpl in *.
  rewrite Heq.
  assert (Heq : dec_exist sub_index_prop x e1 = dec_exist sub_index_prop x e2)
  ; [|rewrite Heq].
  unfold dec_exist. rewrite decide_True.
  assert (dec_exist sub_index_prop x e1)
...
Lemma finite_trace_sub_projection_alt_cons
  (item : composite_transition_item IM)
  (trx : list (composite_transition_item IM))
  : finite_trace_sub_projection_alt (item :: trx) =
    let tail := finite_trace_sub_projection_alt trx in
    match decide (from_sub_projection item) with
    | left e => composite_transition_item_sub_projection item e :: tail
    | _ => tail
    end.
Proof.
  unfold finite_trace_sub_projection_alt at 1.
  match goal with 
  |- context [list_annotate _ _ ?all] =>
    remember all as Hall
  end.
  clear HeqHall.
  match goal with 
  |- context [list_annotate _ ?f _] =>
    remember f as flt
  end.
  simpl in Heqflt.
  unfold bool_decide in Heqflt at 1.
  unfold decide.
  destruct (sub_index_prop_dec (projT1 (l item))) eqn:Hitem.
  - subst flt.
    rewrite list_annotate_unroll.
    rewrite map_cons.
  ; [|subst; reflexivity].
  destruct (dec)
  change (item :: trx) with ([item] ++ trx).
  rewrite (filter_app _ [item] trx).
  unfold filter.
...
Proof.
  ...
  simpl.
  destruct
    (decide (from_sub_projection a)) eqn:Heq.
      j =
